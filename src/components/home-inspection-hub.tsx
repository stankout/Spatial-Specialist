  ...
  return <div className="condition-hub">
    <section className="condition-hero">
      <div className="condition-hero-copy">
        ...
        <div className="condition-hero-actions"><Link className="condition-primary-link" href={`/${locale}/home-inspection/request-inspection`}>{vi ? "Yêu cầu kiểm tra" : "Request an inspection"}<ArrowRight /></Link><Link className="condition-secondary-link" href={`/${locale}/home-inspection/sample-report`}>{vi ? "Hiểu một báo cáo" : "Understand a report"}</Link></div>
      </div>
    ...
    <section className="condition-conversion"><div><p className="eyebrow">{vi ? "Bước tiếp theo" : "Next observation"}</p><h2>{vi ? "Cần kiểm tra tài sản hay hiểu một báo cáo?" : "Inspecting a property—or making sense of a report?"}</h2></div><div><p>{vi ? "Chọn lộ trình phù hợp với giai đoạn hiện tại. Mỗi dịch vụ độc lập với Real Estate và Spatial Consultation." : "Choose the pathway that fits this stage. Each service remains independent from Real Estate and Spatial Consultation."}</p><Link href={`/${locale}/home-inspection/request-inspection`}>{vi ? "Yêu cầu kiểm tra" : "Request an inspection"}<ArrowRight /></Link><Link href={`/${locale}/home-inspection/sample-report`}>{vi ? "Review báo cáo" : "Review a report"}<ArrowRight /></Link></div></section>
