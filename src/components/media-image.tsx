...
import {getMediaRenderPolicy,type MediaRenderContext} from "@/lib/media/presentation";
import type {MediaAsset,MediaSlotKey} from "@/lib/media/types";

export function MediaImage({asset,context,slot,fill=false,sizes="100vw",className,priority=false,alt=""}:{asset:MediaAsset;context:MediaRenderContext;slot?:MediaSlotKey;fill?:boolean;sizes?:string;className?:string;priority?:boolean;alt?:string}){
 const policy=getMediaRenderPolicy(asset,context,slot),style={objectFit:policy.fit,objectPosition:policy.position} satisfies CSSProperties;
