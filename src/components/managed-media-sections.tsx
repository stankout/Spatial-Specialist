...
export function ManagedMediaSections({sections,assets}:{sections:MediaSection[];assets:MediaAsset[]}){
 ...
 return <>{visible.map(section=>{const media=section.assetIds.map(id=>assets.find(item=>item.id===id)).filter((item):item is MediaAsset=>Boolean(item&&getPublicMediaEligibility(item).eligible));if(!media.length)return null;return <section className={`managed-media-section media-section-${section.type}`} key={section.id}>{section.title&&<h2>{section.title}</h2>}<div>{media.map(asset=>asset.mimeType.startsWith("video/")?<video key={asset.id} src={asset.url} controls playsInline preload="metadata"/>:<MediaImage key={asset.id} asset={asset} context="public" alt={asset.alt.en||asset.alt.vi}/>)}</div></section>})}{toolsEnabled&&page&&<OwnerToolbar editHref={editHref} visualPage={visualPage||"homepage"} locale={locale}/>}</>;
}
