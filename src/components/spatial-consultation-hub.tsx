...
export function SpatialConsultationHub({ locale }: { locale: Locale }) {
  ...
  return <div className="space-hub">
    ...
    <div className="space-practical-chapter">
      ...
      <section className="space-selection"><p className="eyebrow">{vi ? "Lựa chọn bất động sản" : "Property selection"}</p><h2>{vi ? "So sánh nhiều hơn giá và vật liệu hoàn thiện." : "Compare more than price and finishes."}</h2><div className="space-comparison-list">{(vi ? ["Bối cảnh vị trí", "Hướng khu đất và ngôi nhà", "Bố cục và lưu thông", "Mức độ phù hợp chức năng", "Quan hệ với môi trường", "Diễn giải truyền thống tùy chọn"] : ["Location context", "Site and house orientation", "Layout and circulation", "Functional suitability", "Environmental relationship", "Optional traditional interpretation"]).map((item, index) => <span key={item}><b>0{index + 1}</b>{item}</span>)}</div><Link className="space-primary-link" href={`/${locale}/spatial-consultation/property-selection`}>{vi ? "Đánh giá lựa chọn tài sản" : "Evaluate property options"}<ArrowRight /></Link></section>
    </div>
