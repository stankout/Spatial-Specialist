...
export async function DELETE(request:Request){if(!getStudioAccess().writable)return denied();try{const id=new URL(request.url).searchParams.get("id");if(!id)throw new Error("Embed ID is required.");const references=await contentReferences("embed",id);if(references.length)throw new Error(`Embed is used by ${references.length} content entr${references.length===1?"y":"ies"}. Remove those references before deleting.`);await deleteEmbed(id);return NextResponse.json({ok:true})}catch(error){return failure(error)}}
