...
export async function DELETE(request:Request){if(!getStudioAccess().writable)return denied();try{const id=new URL(request.url).searchParams.get("id");if(!id)throw new Error("Asset ID is required.");for(const assignment of (await listAssignments()).filter(item=>item.assetId===id))await removeAssignment(assignment.slot);await provider.delete(id);return NextResponse.json({ok:true})}catch(error){return NextResponse.json({ok:false,error:error instanceof Error?error.message:"Delete failed."},{status:400})}}
