...
describe("media library",()=>{
 ...
 it("resolves an owner-controlled About portrait with a safe Homepage legacy fallback",async()=>{const directory=await root(),provider=new LocalMediaStorageProvider(directory),legacy=await provider.upload({buffer:png,filename:"legacy.png",mimeType:"image/png"}),about=await provider.upload({buffer:png,filename:"about.png",mimeType:"image/png"});await approve(provider,legacy.id);await approve(provider,about.id);await assignMedia("homepage.heroPortrait",legacy.id,presentation,directory);expect(await getAboutMedia(directory)).toMatchObject({sourceSlot:"homepage.heroPortrait",asset:{id:legacy.id}});await assignMedia("about.heroPortrait",about.id,presentation,directory);expect(await getAboutMedia(directory)).toMatchObject({sourceSlot:"about.heroPortrait",asset:{id:about.id}})});
